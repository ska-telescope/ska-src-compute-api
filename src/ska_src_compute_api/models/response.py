...
class ProvisionResponse(Response):
    response_code: NonNegativeInt = Field(
        description="Response code; see README", examples=[2]
    )
    response_text: str = Field(
        description="Response specification", examples=["1TB memory not bookable."]
    )
    provision_id: Optional[str] = Field(
        description="Provision ID for claiming", examples=["24-surf-prov"]
    )
    provision_validity: Optional[datetime] = Field(
        description="Validity of the provision",
...
class JobSubmissionResponse(Response):
    ...
    job_id: Optional[str] = Field(
        description="Job ID for status info", examples=["24-surf-job"]
    )


class JobStatusResponse(Response):
    response_code: NonNegativeInt = Field(
        description="Response code; see README", examples=[1]
    )
    response_text: str = Field(
        description="Response specification", example=["Running..."]
    )
    logging: str = Field(
    ...
    output_data: Optional[AnyHttpUrl] = Field(
